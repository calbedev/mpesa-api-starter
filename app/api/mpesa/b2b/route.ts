...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { transactionReference, amount, thirdPartyReference, primaryPartyCode, receiverPartyCode } = body

    // Validate required parameters
    if (!transactionReference || !amount || !thirdPartyReference || !primaryPartyCode || !receiverPartyCode) {
    ...
    const response = await client.makeRequest("POST", 18349, "/ipg/v1x/b2bPayment/", mpesaData)
